...
The server entry point SHALL create an `http.Server` from the Express application and attach a `WebSocketServer` from the `ws` package to that same server. HTTP routes (including `GET /health` and `POST /messages`) and WebSocket connections SHALL share the configured port (`env.PORT`).

#### Scenario: Single port for HTTP and WebSocket

- **WHEN** the server starts successfully
- **THEN** clients MAY connect via HTTP to `/health` and `/messages` and via WebSocket to `ws://host:PORT` on the same port
