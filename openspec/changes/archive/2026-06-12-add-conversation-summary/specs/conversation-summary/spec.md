...
For `type: 'action'` responses, `assistantText` SHALL be a concise string representation (e.g. action name and payload/result). Failed exchanges (`status: 'failed'`) and LLM errors SHALL NOT enqueue a summary job.
...
When the job runs and the conversation already has a `summary`, the worker SHALL load the existing summary, combine it with the current exchange text, call Ollama to produce an updated summary, and SHALL replace `conversation.summary` with the new value.
