...
**Reason**: Sequential numeric `_id` generation via `getNextLocationHistoryId` is replaced by caller-supplied UUID strings, consistent with `messages` and `conversations`.

**Migration**: Callers SHALL generate `_id` with `randomUUID()` from `node:crypto` before calling `insertLocationHistory`. Remove all imports and tests of `getNextLocationHistoryId`.
