# user-preference-repository Specification

## Purpose

Define MongoDB persistence for user preference documents in the `user_preferences` collection: category/value/weight fields linked to `users._id`, and repository CRUD operations. This capability is repository-only — no HTTP or service integration.
