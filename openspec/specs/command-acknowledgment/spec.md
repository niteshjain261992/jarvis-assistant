# command-acknowledgment Specification

## Purpose

Define the pre-command acknowledgment capability: an HTTP endpoint that returns a brief, in-character Jarvis wait message generated by Ollama before command interpretation completes, with the same validation as `POST /command` and explicit failure handling.
